...
3. **Setup Environment:** do your `.env` file in `srcs/` (refer to `DEV_DOC.md` for details). Add the domain name to the `/etc/hosts` which should be `<login>.42.fr` like the subject want, so you should add something like : `127.0.0.1 <login>.42.fr`
...
## *AI Usage*
I used AI, to help me clarify the architecture of the project at the starting, because I was quite lost. I used it too, to understand some parts of the docs. And also, to help me debug the NGINX configuration syntax and some errors. I checked all the commands generated and always asked for more explanations about why and what it would bring me, rather than just copy/past. It's like having a teacher who helps you read the documentation, which can sometimes be intimidating.
...
For this project, I used a `.env` file for simplicity, but Docker Secrets are generally better for security. For example, if a hacker succeeds in gaining access to the server's terminal, they could simply run `docker inspect <name_service>` to reveal all passwords stored in environment variables in plain text. Secrets avoid this exposure.
...
If we don't specify a location to store data, Docker creates volumes in an internal server folder. It acts like a black box which no one know where it is. Specifying a folder (Bind Mount) makes it easier for us to check the data. It also forces us to handle permissions manually. But principally, we use this method because the subject explicitly requires it.
